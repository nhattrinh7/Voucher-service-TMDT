...
export class ValidateVoucherHandler
  implements IQueryHandler<ValidateVoucherQuery, ValidateVoucherResponse>
{
  constructor(
  ...
  async execute(query: ValidateVoucherQuery): Promise<ValidateVoucherResponse> {
    ...
    const userReserved = await this.voucherUsageRepository.countByUserAndVoucher(
      userId,
      voucherId,
      ['RESERVED'],
    )
    ...
    const userConfirmed = await this.voucherUsageRepository.countByUserAndVoucher(
      userId,
      voucherId,
      ['CONFIRMED'],
    )
